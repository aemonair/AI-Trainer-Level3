...
with pdfplumber.open(pdf_path) as pdf:
        ...
        for y0, chars_in_line in lines.items():
            ...
            if colored_chars:
                ...
                if match:
                    ...
                    if num <= 5:
                        print(f"  第{page_num+1}页: 题号{num} → 答案{answer}")
    
    print(f"提取到 {len(single_answers)} 个单选题答案")
    print(f"题号范围: {min(single_answers.keys()) if single_answers else 'N/A'} - {max(single_answers.keys()) if single_answers else 'N/A'}")
    ...
    print(f"题号范围: {min(multiple_answers.keys()) if multiple_answers else 'N/A'} - {max(multiple_answers.keys()) if multiple_answers else 'N/A'}")
