...
ROOT = Path('.').resolve()
chapter = sys.argv[1] if len(sys.argv) > 1 else '1.1.1'
...
# Copy template notebook to time-stamped practice notebook
...
row = [
    ...
    practice_nb.stem,
    f'创建 {chapter} 练习版本（带时间戳）并生成 review 记录',
...
subprocess.run(['git', 'commit', '-m', f'Create timestamped practice notebook for {chapter} and add record ({now})'], check=True)

print(practice_nb.name)
print(practice_md.name)
print('record_added')
